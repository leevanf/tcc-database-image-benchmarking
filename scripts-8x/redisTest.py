...
def insertExecute(image, n, filename):
    ...
    #client.hset(filename, n, image)

def insertBurst(n):
    data = []
    file = open("./image/imageset/metadata/randomOrder.txt", "r")
    ...
    with open('dataBurstRandomRedis.csv', 'w', newline='') as f:
        writer = csv.writer(f)
...
#        readTest(n)
    ...
    with open('dataRandomRedis.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        writer.writerows(data)
        print("Writing line")
