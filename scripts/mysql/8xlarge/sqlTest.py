...
def cursorExecute(cursor, conn, image, n):
...
#    string = '\"' + str(n) + '\"'
    ...
    #cursor.execute('SELECT id FROM test WHERE id=%s', str(n))
...
def insertBurst(n):
    ...
    with open('dataDescendingRandomMysql.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        writer.writerows(data)
        print("Writing line")
...
def insertIndividual(n):
    ...
    for num in range(10):
        ...
        extension2=".jpg"
        createDefaultTable()
        cursor.execute("ALTER TABLE test MODIFY COLUMN id INT UNIQUE auto_increment")
        conn.commit()
        if(n > 0):
            for x in range(n):
                imFile = file.readline().split(' ')
                ...
                mutex.acquire()
                try:
                    time = timeit.timeit(lambda: cursorExecute(cursor, conn, image, x+1), number=1)
                finally:
                    mutex.release()
                if(num == 9):
                    data.append([x, time])
    ...
    with open('dataRandomMysql.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        writer.writerows(data)
        print("Writing line")
...
def insertData(n):
    ...
    try:
        ...
        with open('dataMultiplePython.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(header)
            writer.writerows(data)
            print("Writing line")
